> **XPENC fork notice:** this is a local, patched copy of
> [`flutter_tesseract_ocr` 0.4.31](https://pub.dev/packages/flutter_tesseract_ocr)
> (BSD-3), vendored here instead of pulled from pub.dev. The upstream
> package bundles a prebuilt `tesseract4android-release.aar` directly in
> `android/libs/` via a local `flatDir` repo — an unreviewable binary blob
> that fails F-Droid's build policy (see XPENC's GitHub issue #57). This
> fork removes that file and resolves the identical library
> (`cz.adaptech.tesseract4android:tesseract4android`, Apache-2.0) from
> JitPack instead, which F-Droid's Inclusion Policy explicitly allows.
> Everything else — Dart API, iOS/web sources — is untouched. See
> `android/build.gradle` for the exact change.
